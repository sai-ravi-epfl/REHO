...
def plot_stacked_costs(results, era, additional_costs=dict(), additional_gwp=np.array([0]), idx=None, annotations=None, x_label= "Data Center Size [%]", y_lim=[65, 200, 1.0], save_name="bi", pareto_ids = 32):
    ...
    if idx is None:
        ...
        idx = np.linspace(0, 200,  len(results[0]['gwp'].keys()))
    costs = {}
    ...
    for n, res in enumerate(results):
        ...
        ax2.set_ylim([5, y_lim[1]])
        '''
        # Get Heat Ratio
        heat_ratios = get_heat_ratio_df(results)

        # Create twin axis for heat ratio
        ax3 = ax[n].twinx()

        # Interpolation for smooth curve
        x_smooth = np.linspace(idx.min(), idx.max(), 300)  # Create a smooth range of x-values
        y_smooth = make_interp_spline(idx, heat_ratios[n][indexes], k=3)(x_smooth)  # Cubic spline interpolation

        # Plot smooth curve
        ax3.plot(x_smooth, y_smooth, color="tab:blue", linestyle="-", label="Heat Ratio")

        # Formatting the secondary axis
        if n == len(results) - 1:
            ax3.set_ylabel('Heat Ratio [-]', fontsize=16)
        ...
        ax3 = ax[n].twinx()
        ax3.plot(idx, heat_ratios[n][indexes], color="darkred", linestyle="-", label="Heat Ratio")

        if n==len(results)-1:
            ax3.set_ylabel('Heat Ratio [-]', fontsize=16)
            ax3.spines["right"].set_position(("axes", 1.2))
            ax3.tick_params(labelsize=12)#, labelcolor="#a40d0d", color="#a40d0d")
        else:
            ax3.tick_params(right= False, labelright=False)

        # Set y-labels and colors
        #ax3.set_ylabel('Heat Ratio (Q_DC / Q_building)', color="tab:blue", fontsize=14)
        #ax3.tick_params(axis='y', labelcolor="tab:blue")

        # Adjust rightmost spine to prevent overlap


        # Ensure y-limits are properly set for both ax2 and ax3
        ax3.set_ylim([0, y_lim[2]])
        ax2.set_ylim([5, y_lim[1]])

        # Align y-axes properly
        align.yaxes(ax2, 0, ax3, 0, 0.112)
        ...
        annotation = ['(A)', '(B)', '(C)'][n]
        ax[n].text(idx[1] * 0.5, 0.91 * y_lim[0], annotation, color="black", fontsize=20)

        if annotations is not None:
            ax[n].text(idx[-1] * 0.25, 0.91 * y_lim[0], annotations[n], color="black", fontsize=14)
        ...
        handles, labels = [], []
        for axis in [ax[n], ax2, ax3]:  # oder deine Achsenliste
...
def plot():
    ...
    plot_stacked_costs([uni, bi], era, annotations=annotations, save_name="EPFL_T_S", pareto_ids= len(list(uni['gwp'].keys())) )
