...
var Demand_Active{u in UnitsOfType['STES'], p in Period} binary;
param BigM{u in UnitsOfType['STES']} default 1e8;
param small_M{u in UnitsOfType['STES']} default  10;
#var Active_DHN{v in UnitsOfType['DHN_hex'], p in Period, t in Time[p]} binary;
...
subject to BAT_c3_IP{u in UnitsOfType['STES'],p in Period,t in Time[p]}:
Units_demand['Heat',u,p,t]*dt[p] <= (STES_limit_ch_IP[u]-STES_limit_di_IP[u])*Units_Mult[u];

subject to BAT_c4_IP{u in UnitsOfType['STES'],p in Period,t in Time[p]}:
Units_supply['Heat',u,p,t]*dt[p] <= (STES_limit_ch_IP[u]-STES_limit_di_IP[u])*Units_Mult[u];

subject to BAT_c7_IP{u in UnitsOfType['STES'], p in Period, t in Time[p]}:
    Units_demand['Heat', u, p, t] <= Demand_Active[u, p] * BigM[u];

subject to BAT_c8_IP{u in UnitsOfType['STES'], p in Period, t in Time[p]}:
    Units_supply['Heat', u, p, t] <= (1 - Demand_Active[u, p]) * BigM[u] + small_M[u];

subject to BAT_c9_IP{u in UnitsOfType['STES'], p in Period, t in Time[p]}:
    Units_supply['Heat', u, p, t] <= Demand_Active[u, p] * BigM[u];

subject to BAT_c10_IP{u in UnitsOfType['STES'], p in Period, t in Time[p]}:
    Units_demand['Heat', u, p, t] <= (1 - Demand_Active[u, p]) * BigM[u] + small_M[u];
