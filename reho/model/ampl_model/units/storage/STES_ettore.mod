...
/*
# constraint to impose that all the energy going to the storage is coming from ThermalSolar_district
...
STES_E_stored_IP[u,first(Year)] = STES_self_discharge_IP[u]*STES_E_stored_IP[u,last(Year)] +
	(STES_efficiency_IP[u]*Units_demand['Heat',u,PeriodOfYear[last(Year)],TimeOfYear[last(Year)]] - (1/STES_efficiency_IP[u])*Units_supply['Heat',u,PeriodOfYear[last(Year)],TimeOfYear[last(Year)]])*dt[PeriodOfYear[last(Year)]];


param delta_T_STES{u in UnitsOfType['STES']} default 60;

# constraint to impose the minimum allowed temperature inside the storage
subject to min_T_allowed{u in UnitsOfType['STES'], hy in Year}:
	Units_Mult[u]*Text_Geneva[hy]+STES_E_stored_IP[u,hy]*delta_T_STES[u] >= Units_Mult[u]*22.5;
...
subject to STES_E_stored_IP_cyclic{u in UnitsOfType['STES']}:
STES_E_stored_IP[u,first(Year)] = STES_self_discharge_IP[u]*STES_E_stored_IP[u,last(Year)] +
	(STES_efficiency_ch_IP[u]*Units_demand['Heat',u,PeriodOfYear[last(Year)],TimeOfYear[last(Year)]] - (1/STES_efficiency_di_IP[u])*Units_supply['Heat',u,PeriodOfYear[last(Year)],TimeOfYear[last(Year)]])*dt[PeriodOfYear[last(Year)]];
