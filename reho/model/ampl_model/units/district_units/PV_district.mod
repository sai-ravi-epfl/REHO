...
subject to PVA_c1{u in UnitsOfType['PV'],p in Period,t in Time[p]}:
Units_supply['Electricity',u,p,t] = PVA_inverter_eff[u]*PVA_efficiency[u,p,t]*(I_global[p,t]/1000)*(Units_Mult[u]/PVA_efficiency_ref[u])- Units_curtailment['Electricity', u, p,t];

subject to PVA_c2{u in UnitsOfType['PV']}:
(Units_Mult[u]/PVA_efficiency_ref[u]) = PVA_module_size[u]*PVA_module_nbr[u];

subject to PVA_c3{u in UnitsOfType['PV'],p in Period,t in Time[p]}:
Units_curtailment['Electricity', u, p,t] <= PVA_inverter_eff[u]*PVA_efficiency[u,p,t]*(I_global[p,t]/1000)*(Units_Mult[u]/PVA_efficiency_ref[u]);
...
#sum{ui in UnitsOfType['ThermalSolar'] inter UnitsOfHouse[h]}(Units_Mult[ui]) + sum{uj in UnitsOfType['PV'] inter UnitsOfHouse[h]}(Units_Mult[uj]/PVA_efficiency_ref[uj]) = ((SolarRoofArea[h]) div PVA_module_size[u])*PVA_module_size[u]; #adding 0.083 to limit the capacity of the PV in each roof (EPFL case study)
