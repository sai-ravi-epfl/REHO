...
def plot_pareto(results, color='ColorPastel', return_df=False):
    """
...
def plot_pareto_by_objectives(results, objectives=["CAPEX", "OPEX"], style='plotly', annotation="TOTEX", filename=None, export_format='png'):
    """
...
def plot_SOC_STES(df_Results, resolution='daily'):
...
def plot_energy_balance_for_STES(df_Results, units_to_plot, color='ColorPastel', day_of_the_year = '' ,time_range='week', label='EN_long'):

# it is used to see the energy flow of each unit
# (you need to put in the main script the unit that are included by the optimization)
# you can select the amount of day you want to see in the plot and which is the starting day
    ...
    idx = list(range(1, len(import_profile["Heat"])))

    fig = go.Figure()

    if export_profile["Heat"].any() > 0:
        fig.add_trace(go.Scatter(
            x=idx,
            y=-export_profile["Heat"],
            mode="lines",
            name='Heat_export',
    ...
    if import_profile["Heat"].any() > 0:
        fig.add_trace(go.Scatter(
            x=idx,
            y=import_profile["Heat"],
            mode="lines",
            name='Heat_import',
            line=dict(color="blue", dash="dot")
        ))
    ...
    #fig.update_yaxes(title_text="Heat flows [kW]")
    fig.update_layout(
        title="Heat flows and long term storage behaviour for "+str(time_range)+" starting form day: "+str(day_of_the_year),
        xaxis=dict(
...
def plot_c_d_STES(df_Results):
    ...
    # it is used to evaluate the amount of energy that is charged and discarched each month in the seasonal thermal storage
